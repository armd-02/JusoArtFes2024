class modal_Activities {
	...
	// 画像をピンチイン/アウト	view:true=表示/false=非表示
	pinchImage(view) {
		...
		realX = parseInt(image.style.left);
		realY = parseInt(image.style.top);
		org_W = parseInt(image.style.width);
		org_H = parseInt(image.style.height);
		...
		function handleTouchMove(e) {
			...
			if (e.touches.length == 1) {		// 1本指の時
				...
				var distance = Math.hypot(e.touches[0].pageX - e.touches[1].pageX, e.touches[0].pageY - e.touches[1].pageY);
				...
				pinchStartDistance = distance
				let subX = (orgX - ((scale - orgScale) * orgX) / 2)
				let subY = (orgY - ((scale - orgScale) * orgY) / 2)
				image.style.left = subX + "px";
				image.style.top = subY + "px";
				image.style.width = scale * org_W + "px"
				image.style.height = scale * org_H + "px"
			}
		}
